...
async def authenticate(request: AuthenticationRequest, user_agent: Optional[str] = Header(None), client_host: Optional[str] = Header(None)):
    ...
    try:
        ...
        if isinstance(e, ChallengeExpiredError):
            ...
            status_code = 401
            detail = f"Invalid signature provided for address {request.evrmore_address}. Please ensure you are signing the exact challenge text with the correct wallet."
